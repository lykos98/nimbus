...
int send_to_api(char* message)
{
  String url = String(API_URL) + String(STATION_ID) + "/data";
...
void bmp_sleep() {
#ifdef BME
  bmp.setSampling(Adafruit_BME280::MODE_SLEEP);
#else
 ...
 int tries = 0;
  ...
  if(tries == MAX_N_TRIES)
  {
    snprintf( message, LOG_BUFFER_SIZE, 
              "{\"stationId\":\"%s\",\"error\":\"Cannot initialize BMP\",\"secret\":\"%s\"}", STATION_ID, SECRET);
    send_to_api(message);
    delay(500);
  ...
  {
    snprintf( message, LOG_BUFFER_SIZE,
              "{\"stationId\":\"%s\",\"error\":\"Cannot initialize AHT\",\"secret\":\"%s\"}", STATION_ID, SECRET);
    send_to_api(message);
    delay(500);
    esp_sleep_enable_timer_wakeup(SLEEP_TIME * 1000 * 1000);
    esp_deep_sleep_start();
  ...
  snprintf(message, LOG_BUFFER_SIZE,
            ...
            "\"batteryV\":%.2f,"
            "\"RSSI\":%ld, \"secret\":\"%s\"}",
          STATION_ID,h,t,p,solar_voltage, battery_voltage, WiFi.RSSI(), SECRET); 
  LOGF("Message built");
  //LOGF(message);
  int code = send_to_api(message);
  LOGF("Message send with code: %d", code);
  delay(100);
