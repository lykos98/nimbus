...
void setup() {
  ...
  esp_err_t gpio_config_err = -1;
  int tries = 0;
  while(gpio_config_err != ESP_OK || tries <= MAX_N_TRIES)
  {
    gpio_config_err = ESP_OK;
    gpio_config_err = gpio_config_err || rtc_gpio_deinit(ANEMOMETER_PIN);
    gpio_config_err = gpio_config_err || rtc_gpio_init(ANEMOMETER_PIN);
    gpio_config_err = gpio_config_err || rtc_gpio_set_direction(ANEMOMETER_PIN, RTC_GPIO_MODE_INPUT_ONLY);
    gpio_config_err = gpio_config_err || rtc_gpio_pulldown_dis(ANEMOMETER_PIN);     // enable the pull-up resistor
    gpio_config_err = gpio_config_err || rtc_gpio_pullup_en(ANEMOMETER_PIN);  // disable the pull-down resistor
    gpio_config_err = gpio_config_err || rtc_gpio_hold_en(ANEMOMETER_PIN);       // required to maintain pull-up
    tries++;
  }
  ...
  if (reason == ESP_RST_DEEPSLEEP) {
    ...
    ulp_active           = RTC_SLOW_MEM[ULP_ACTIVE] & 0xFFFF;
    LOGF("ulp active %u\n", ulp_active);
    
    RTC_SLOW_MEM[COUNTER] = 0;   
    RTC_SLOW_MEM[OLD_STATE] = 0; 
    RTC_SLOW_MEM[ULP_ACTIVE] = 0; 
  } else {
    // First-time setup
    RTC_SLOW_MEM[COUNTER] = 0;
    RTC_SLOW_MEM[OLD_STATE] = 0;
  ...
  snprintf(message, LOG_BUFFER_SIZE,
          ...
          wind_speed, wind_angle, ulp_err, ulp_active, gpio_config_err, str_reason); 
  LOGF("Message built");
